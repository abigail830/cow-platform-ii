...
async function main() {
  ...
  await client.agents.send('content-studio', instanceId, {
    message: 'Reply with exactly: smoke-ok',
  });

  let streamText = '';
  try {
    streamText = await waitForAssistantText(client, 'content-studio', instanceId, '');
  } catch (e) {
  ...
  const history = await client.agents.history('content-studio', instanceId);
  ...
  let turn2 = '';
  try {
    turn2 = await waitForAssistantText(client, 'content-studio', instanceId, '');
  } catch (e) {
