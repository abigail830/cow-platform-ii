...
import { z } from 'zod';
import {
  listPlatformAgentDirs,
  loadPlatformMcpTemplate,
  listAssetSummaries,
} from '../../../agent-assets/manifest.ts';
import { parseMcpServersJson } from '../../../agent-assets/parse-mcp-servers.ts';
import { assertSkillIdsAccessible, listVisibleSkillsForUser } from '../../../skills/application/skills.ts';
import skillsRouter from './skills.ts';
import { ensureAgentCatalogReady, upsertStudioAgentInRegistry } from '../../../agents/catalog/boot.ts';
import { loadAgentSpec } from '../../../agents/catalog/discover.ts';
import { listPlatformMcpDiscoveredTools } from '../../../agents/catalog/load-mcp.ts';
import { getAgentRegistry } from '../../../agents/catalog/registry.ts';
import { invalidateCatalogAgentRuntimeCache, resolveCatalogAgentRuntime } from '../../../agents/catalog/resolve-agent-runtime.ts';
...
import { userHasStudioAgentAccess } from '../../../auth/resource-access.ts';
...
import { reloadFlueRuntimeFromRegistry } from '../../../agents/runtime/init.ts';
import {
  hasStoredModelConfigApiKey,
  sealModelConfigApiKeyForStorage,
} from '../../../model-config/infrastructure/model-config-secret.ts';
import {
  assertDatasourceIdsOwnedByUser,
  createDatasourceForUser,
  deleteDatasourceForUser,
  listDatasourcesForUser,
} from '../../../tools/database-mcp/datasource-service.ts';
...
studio.get('/agents/:id/access', requireAuth, async (c) => {
  ...
  return handleGetResourceAccess(c, 'studio_agent', id);
});

studio.put('/agents/:id/access', requireAuth, async (c) => {
  const id = c.req.param('id');
  if (!id) return c.json({ error: 'Agent id is required' }, 400);
  return handlePutResourceAccess(c, 'studio_agent', id);
});
