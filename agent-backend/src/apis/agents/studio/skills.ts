...
import { initSkillUpload } from '../../../skills/application/skill-upload.ts';
import { defaultSkillPreviewPath, listSkillTree, readSkillFile } from '../../../agent-assets/skill-browse.ts';
import {
  ...
  resolveUserSkillIdForAccess,
} from '../../../skills/application/skills.ts';
import {
...
skills.get(
  ...
  requireResourcePermission('agent', 'asset-market', 'read'),
  async (c) => {
    const skillId = await resolveUserSkillIdForAccess(c.req.param('id'));
    if (!skillId) return c.json({ error: 'Skill not found' }, 404);
    return handleGetResourceAccess(c, 'skill', skillId);
  },
);
...
skills.put(
  ...
  async (c) => {
    const skillId = await resolveUserSkillIdForAccess(c.req.param('id'));
    if (!skillId) return c.json({ error: 'Skill not found' }, 404);
    return handlePutResourceAccess(c, 'skill', skillId);
  },
);
