import type { DocumentCaptureInputMode } from '../../infrastructure/db/index.ts';
import type { CaptureStatusSegment } from '../domain/capture/capture-status-resolve.ts';
import { getLatestPipelineJobsForSegments } from '../../pipeline/application/pipeline-jobs.ts';
...
export async function syncDocumentCaptureStatus(captureId: string) {
  ...
  const { appDocumentCaptureSegments } = await import('../../infrastructure/db/index.ts');
