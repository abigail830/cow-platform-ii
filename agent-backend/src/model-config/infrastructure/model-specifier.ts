...
const CHAT_COMPLETIONS_API_TYPES = new Set(['chat-completions']);
...
function resolveApiSlug(config: RuntimeModelConfig): string {
  ...
  if (CHAT_COMPLETIONS_API_TYPES.has(config.apiType)) return 'openai-completions';
  throw new Error(
    `Model config "${config.name}" uses apiType "${config.apiType}" which is not supported for chat-completions callers`,
  );
}
...
/** Prefer known catalog provider IDs so tool-capable models keep full metadata. */
...
function registerConfigProvider(config: RuntimeModelConfig): string {
  ...
  void requireBaseUrl(config);
  void resolveApiSlug(config);
  return `${providerId}/${modelId}`;
}

/** Map an Admin model config row to a provider/model specifier string. */
export function resolveModelSpecifierFromConfig(config: RuntimeModelConfig): string {
  if (!CHAT_COMPLETIONS_API_TYPES.has(config.apiType)) {
    throw new Error(
      `Model config "${config.name}" must be apiType chat-completions for chat-completions callers`,
    );
  }
