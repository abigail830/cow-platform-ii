...
import { parseSkillZipBuffer } from '../domain/parse-skill-zip.ts';
import { getReservedSkillNames, isUuidSkillId } from '../domain/reserved-names.ts';
import { dispatchSkillImport } from './skill-import.ts';
import { validateSkillUploadComplete } from './skill-upload.ts';
import { MAX_SKILL_EXTRACTED_BYTES } from '../domain/skill-limits.ts';
...
/** Resolve a user-owned skill UUID for access routes without loading full skill detail. */
export async function resolveUserSkillIdForAccess(skillRef: string): Promise<string | null> {
  const row = await getSkillRowByRef(skillRef);
  if (!row || row.origin !== 'user') return null;
  return row.id;
}
