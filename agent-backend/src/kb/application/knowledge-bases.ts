...
} from '../domain/kb-import-limits.ts';
import { resolveDefaultPipelineIdForKbType } from '../domain/kb-pipeline-binding.ts';
import { resolvePipelineConfigYamlSnapshot } from '../../pipeline/domain/pipeline-default-config.ts';
import { isServerlessRuntime } from '../../pipeline/application/pipeline-worker-mode.ts';
import { getPipelineConfigById, getPipelineConfigByPipelineName } from '../../pipeline/infrastructure/pipeline-config-store.ts';
import { getModelConfigById } from '../../model-config/infrastructure/model-config-store.ts';
...
import { KB_IMPORT_JOB_KIND_TO_PIPELINE } from '../../pipeline/domain/pipeline-catalog.ts';
import { resolveFaqIndexEmbedConfigForKb } from './faq-index-workflow.ts';
import { pipelineSupportsFaqJobKind } from '../domain/faq-pipeline-binding.ts';
...
function toKbItemPublic(
  ...
  return {
    ...
    original_download_path: `/api/knowledge/documents/${row.documentId}/download`,
  ...
  if ((input.type === 'faq' || input.type === 'rag') && row) {
    try {
      if (input.type === 'faq') {
        ...
        const { resolveRagIndexEmbedConfigForKb } = await import('./rag-index-workflow.ts');
...
export async function updateKnowledgeBase(
  ...
  if (input.pipeline_id !== undefined && (row.type === 'faq' || row.type === 'rag')) {
    if (row.type === 'faq') {
      const { resolveFaqIndexPipeline } = await import('../domain/faq-pipeline-binding.ts');
      const indexPipeline = await resolveFaqIndexPipeline({
        pipelineId: input.pipeline_id?.trim() || null,
      });
      nextPipelineId = indexPipeline.id;
    } else {
      const { resolveRagIndexPipeline } = await import('../domain/rag-pipeline-binding.ts');
  ...
  if (input.faq_settings !== undefined && row.type === 'faq') {
    ...
    if (extractId !== undefined && extractId !== null && extractId.trim()) {
      const { resolveFaqExtractPipeline } = await import('../domain/faq-pipeline-binding.ts');
  ...
  if (
    ...
    try {
      const { getPipelineConfigById: getPipe } = await import('../../pipeline/infrastructure/pipeline-config-store.ts');
      const pipe = updated.pipelineId ? await getPipe(updated.pipelineId) : null;
      if (pipe) {
        const embed =
          row.type === 'faq'
            ? await (
                await import('./faq-index-workflow.ts')
              ).resolveFaqIndexEmbedConfig(pipe)
            : await (
                await import('./rag-index-workflow.ts')
...
export async function upsertKbItemFromWorker(
  ...
  if (input.markdown_s3_key) {
    if (isServerlessRuntime()) {
      ...
      const { readStorageText } = await import('../../infrastructure/oss/storage-read.ts');
  ...
  const { materializeDiscoveryFields } = await import('../../tools/pageindex-search/discovery-materialize.ts');
...
export async function startKbRagIndexImport(input: {
  ...
  const { resolveRagIndexPipeline } = await import('../domain/rag-pipeline-binding.ts');
  const { resolveRagIndexEmbedConfig } = await import('./rag-index-workflow.ts');
