...
import { buildChannelPath, collectDescendantIds } from '../channels/channel-tree.ts';
import { getChannelById, getDocumentById } from '../documents/documents.ts';
...
import { KB_IMPORT_JOB_KIND_TO_PIPELINE } from '../../shared/pipeline/pipeline-catalog.ts';
import { resolveFaqIndexEmbedConfigForKb } from '../../shared/kb/faq-index-workflow.ts';
import { pipelineSupportsFaqJobKind } from '../../shared/kb/faq-pipeline-binding.ts';
  ...
  if ((input.type === 'faq' || input.type === 'rag') && row) {
    try {
      if (input.type === 'faq') {
        ...
        const { resolveRagIndexEmbedConfigForKb } = await import('../../shared/kb/rag-index-workflow.ts');
...
export async function updateKnowledgeBase(
  ...
  if (input.pipeline_id !== undefined && (row.type === 'faq' || row.type === 'rag')) {
    if (row.type === 'faq') {
      const { resolveFaqIndexPipeline } = await import('../../shared/kb/faq-pipeline-binding.ts');
      const indexPipeline = await resolveFaqIndexPipeline({
        pipelineId: input.pipeline_id?.trim() || null,
      });
      nextPipelineId = indexPipeline.id;
    } else {
      const { resolveRagIndexPipeline } = await import('../../shared/kb/rag-pipeline-binding.ts');
  ...
  if (input.faq_settings !== undefined && row.type === 'faq') {
    ...
    if (extractId !== undefined && extractId !== null && extractId.trim()) {
      const { resolveFaqExtractPipeline } = await import('../../shared/kb/faq-pipeline-binding.ts');
  ...
  if (
    ...
    try {
      const { getPipelineConfigById: getPipe } = await import('../../shared/pipeline/pipeline-config-store.ts');
      const pipe = updated.pipelineId ? await getPipe(updated.pipelineId) : null;
      if (pipe) {
        const embed =
          row.type === 'faq'
            ? await (
                await import('../../shared/kb/faq-index-workflow.ts')
              ).resolveFaqIndexEmbedConfig(pipe)
            : await (
                await import('../../shared/kb/rag-index-workflow.ts')
...
export async function startKbRagIndexImport(input: {
  ...
  const { resolveRagIndexPipeline } = await import('../../shared/kb/rag-pipeline-binding.ts');
  const { resolveRagIndexEmbedConfig } = await import('../../shared/kb/rag-index-workflow.ts');
