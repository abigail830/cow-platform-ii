...
import { appEvalDatasetItems, appEvalRunItems, db, type PipelineJobStage } from '../../infrastructure/db/index.ts';
import {
  evalRunDocumentMarkdownKey,
  evalRunDocumentParseResultKey,
} from '../../eval/infrastructure/eval-run-files.ts';
import {
  createPipelineJob,
  getPipelineJobById,
...
export async function reconcileEvalDocumentPipelineJobsForRun(runId: string): Promise<void> {
  ...
  const { shouldFailStaleDocumentPipelineJob } = await import('../../pipeline/application/document-pipeline-stale.ts');
...
): Promise<typeof import('../../infrastructure/db/index.ts').appPipelineJobs.$inferSelect> {
...
export async function buildEvalLinkedDocumentPipelineContextOverrides(
  ...
  const { getS3Config } = await import('../../infrastructure/oss/s3-config.ts');
