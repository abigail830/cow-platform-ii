...
import { getCatalogFlueAgentModules } from '../catalog/boot.ts';
import { getCatalogA2aChannelModules } from '../a2a/create-channel.ts';
import db from '../../db.ts';
import { setPlatformFlueStores } from './platform-flue-stores.ts';
...
async function runFlueRuntimeInit(): Promise<void> {
  ...
  const { bootAgentCatalogAsync } = await import('../catalog/boot.ts');
...
export function resetFlueRuntimeInitForTests(): void {
  initialized = false;
  initPromise = undefined;
}
