...
import { refreshModelConfigCache } from '../../model-config/infrastructure/model-registry.ts';
...
/** Await full catalog boot (FS agents + studio rows). Use in HTTP handlers instead of bootAgentCatalog(). */
export async function ensureAgentCatalogReady(): Promise<void> {
...
> {
  if (!booted) {
    throw new Error('Agent catalog not ready. Call ensureAgentCatalogReady() or bootAgentCatalogAsync() first.');
  }
