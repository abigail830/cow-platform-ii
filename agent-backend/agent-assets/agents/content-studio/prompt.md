...
3. **Use the sandbox** for scripts, conversions, unzip/rezip, thumbnails, and validation commands referenced in the skill. Run skill scripts from **`/home/user/content-studio/skills/<skill>/scripts/`** — do **not** use `/.flue/packaged-skills/` in bash (that path is only for Flue `read` / `activate_skill`, not shell commands).
4. **Deliver artifacts**, not only prose: write files to the workspace, name them clearly, run QA, then **publish** the final file(s) with `publish_artifact` so the user gets a platform download link — not only a sandbox path.
5. **Quality bar**: for docx/pptx, run the skill's verification steps (PDF render, thumbnails, validate.py) when applicable **before** calling `publish_artifact` on the final deliverable.
