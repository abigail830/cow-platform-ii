...
export function PipelinesConfigPage() {
  ...
  const [systemPipelines, setSystemPipelines] = useState<PipelineConfig[]>([]);
  ...
  const loadPipelines = useCallback(async () => {
    ...
    try {
      ...
      setSystemPipelines(data.system_pipelines ?? []);
      setTotal(data.total);
    } catch (err) {
      ...
      <main className="admin-page">
        <header className="admin-header">
          ...
          <AdminPageDescription>
            Document pipelines parse channel uploads. PageIndex knowledge bases use the built-in KB
        ...
        <div className="admin-table-wrap">
          <table className="admin-table pipelines-config-table">
            ...
            <tbody>
              {loading ? (
              ...
              ) : pipelines.length === 0 && systemPipelines.length === 0 ? (
                <tr>
                  <td colSpan={7} className="admin-table-empty">
                ...
                <>
                  ...
                  {systemPipelines.map((pipeline) => (
                    <tr key={pipeline.id} className="pipeline-system-row">
                      <td>
                        <div className="model-cell">
                          <span className="model-cell-icon" aria-hidden>
                            <GitBranch {...iconProps()} />
                          ...
                          <div className="model-cell-name">
                            {pipeline.name}
                            <span className="pipeline-system-badge">System</span>
                          </div>
                        </div>
                      </td>
                      <td className="pipeline-desc-cell pipeline-multiline-cell" title={pipeline.description ?? undefined}>
                        {pipeline.description}
                        {pipeline.boundTo && (
                          <div className="pipeline-bound-hint">Bound to: {pipeline.boundTo}</div>
                        )}
                      </td>
                      <td>—</td>
                      <td className="pipeline-command-cell pipeline-multiline-cell" title={pipeline.commandTemplate}>
                      ...
                      <td className="pipeline-system-meta">Built-in</td>
                      <td>
                        <label className="form-checkbox pipeline-enabled-toggle">
                          <input
                            type="checkbox"
                            ...
                            checked={pipeline.isEnabled}
                            disabled
                            aria-label="Enabled (system pipeline)"
                          />
                        </label>
                      </td>
                      <td className="pipeline-actions-cell">
                        <span className="admin-muted pipeline-system-meta">—</span>
                      </td>
                    </tr>
                  ))}
                </>
        ...
        <footer className="admin-footer">
          <span>
            ...
            {systemPipelines.length > 0 ? ` (+ ${systemPipelines.length} system)` : ''}
          </span>
