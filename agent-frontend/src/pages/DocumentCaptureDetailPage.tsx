...
import {
  captureAwaitingSegmentProcessing,
...
export function DocumentCaptureDetailPage() {
  ...
  function segmentUploadFn(inputMode: DocumentCaptureInputMode) {
    if (inputMode === 'transcript') return uploadCaptureTranscriptSegment;
    if (inputMode === 'audio') return uploadCaptureAudioSegment;
    return uploadCaptureDocumentSegment;
  }

  function formatSegmentBytes(sizeBytes: number): string {
    return formatDocumentBytes(sizeBytes);
  }
  ...
  const loadCapture = useCallback(
    async (options?: { silent?: boolean; sync?: boolean }) => {
      ...
      try {
        ...
        const data = await getDocumentCapture(captureId, { sync });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void getDocumentCapture(captureId, { sync: true })
      .then((data) => {
  ...
  async function handleSaveDetails(input: {
    ...
    if (!captureId) return;
    setError('');
    const updated = await updateDocumentCapture(captureId, input);
    setCapture(updated);
  }

  async function handleProcessSegment(segmentId: string) {
    setTranscribingSegmentIds((current) => new Set(current).add(segmentId));
    setError('');
    try {
      await runCaptureSegmentPipeline(captureId!, segmentId);
      await loadCapture({ silent: true });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to start segment processing');
    } finally {
  ...
  async function handleProcessAllSegments() {
    if (!capture?.segments.length) return;
    const targets = capture.segments.filter((segment) =>
      segmentNeedsProcessing(segment, capture.input_mode),
    );
    ...
    try {
      await Promise.all(
        targets.map((segment) => runCaptureSegmentPipeline(capture.id, segment.id)),
      );
      await loadCapture({ silent: true });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to start segment processing');
  ...
  async function handleUploadSegment(file: File) {
    ...
    setError('');
    try {
      const upload = segmentUploadFn(capture.input_mode);
  ...
  const segmentAcceptTypes = transcriptCapture
    ? '.md,.markdown,.docx'
    : documentFileCapture
      ? '.pdf,.png,.jpg,.jpeg,.webp,.docx,.pptx,.xlsx,.epub,.xmind,.md,.markdown'
      : '.m4a,.mp3,.wav,.flac,.aac,.amr,.ogg,.opus,.webm';
  const segmentsNeedingProcessing = capture.segments.filter((segment) =>
    segmentNeedsProcessing(segment, capture.input_mode),
  );
  const awaitingSegmentProcessing = captureAwaitingSegmentProcessing(capture, capture.segments);
  const processAllLabel = documentFileCapture ? 'Process all' : 'Transcribe all';
  const statusColumnLabel = transcriptCapture ? 'Source' : documentFileCapture ? 'Parse' : 'ASR';
  ...
  function renderActiveArtifactTabContent(currentCapture: DocumentCaptureDetail): ReactNode {
    if (activeTabLoading) {
      return (
        <PanelLoading
          label={
      ...
      <div className="document-detail-toolbar">
        <div className="document-detail-title-row">
          ...
          <span className="document-status-badge capture-input-mode-badge">
            {DOCUMENT_CAPTURE_INPUT_MODE_LABELS[capture.input_mode ?? 'document']}
          </span>
        </div>
      </div>
      ...
      <div className="audio-detail-layout">
        ...
        {awaitingSegmentProcessing ? (
          <p className="capture-detail-hint" role="status">
            {segmentsNeedingProcessing.length} segment
            {segmentsNeedingProcessing.length === 1 ? '' : 's'} still need processing. Use{' '}
            <strong>{processAllLabel}</strong> below, or run processing on individual segments.
          </p>
        ) : null}
              ...
              <div className="document-detail-toolbar-actions">
                ...
                {!transcriptCapture ? (
                  <button
                    ...
                    onClick={() => void handleProcessAllSegments()}
                  >
          ...
          {capture.segments.length === 0 ? (
            <p className="document-detail-panel-empty">
              {transcriptCapture
                ...
                : documentFileCapture
                  ? 'No segments yet. Add document files to continue.'
              ...
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    <th>Name</th>
                    <th>Size</th>
                    <th className="documents-status-col">{statusColumnLabel}</th>
                          ...
                          {canWriteCapture && !transcriptCapture ? (
                            <>
                              <button
                                type="button"
                                className="icon-btn"
                                title={documentFileCapture ? 'Run parse' : 'Transcribe segment'}
                                aria-label={`Process ${segment.name}`}
                                disabled={transcribingSegmentIds.has(segment.id)}
                                onClick={() => void handleProcessSegment(segment.id)}
                              >
              ...
              {canWriteCapture && !artifactPreviewMaximized ? (
                <div className="document-detail-toolbar-actions">
                  <button
                    ...
                    title={
                      !canRunPostProcess && awaitingSegmentProcessing
