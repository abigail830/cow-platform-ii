"""Internal API client for async audio pipeline jobs."""
...
class AudioPipelineJobApiError(RuntimeError):
    """Raised when an audio pipeline job API call fails."""


def _auth() -> tuple[dict[str, str], tuple[str, str] | None]:
    cred = try_api_request_auth()
    if cred is None:
        raise AudioPipelineJobApiError(
...
def _request(
    ...
    *,
    ...
    for attempt in range(2):
        ...
        raise AudioPipelineJobApiError(f"{method} {url} failed ({resp.status_code}): {detail}")
    return {}


def get_audio_job_context(api_url: str, job_id: str) -> dict[str, Any]:
    base = api_url.rstrip("/")
    return _request("GET", f"{base}/internal-api/audio-pipeline/jobs/{job_id}")
...
def post_audio_provider_ready(
    ...
    *,
    ...
    return _request(
        "POST",
        f"{base}/internal-api/audio-pipeline/jobs/{job_id}/events",
        json_body=body,
    )
