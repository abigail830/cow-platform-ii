...
def finalize_baidu_task(
    *,
    ...
    """Download Baidu parse results for an existing task and build result.json."""
    cfg_settings = None
    api_key = ""
    secret_key = ""
    try:
        ...
        secret_key = cfg_settings.baidu_cloud_secret_key
    except Exception:
        pass
    if not api_key or not secret_key:
    ...
    task_result = query_parse_task(token, task_id, session=session)

    status = str(task_result.get("status", "")).lower()
    if status != "success":
        raise BaiduParseError(f"Baidu task not ready (status={status})")
    ...
    if result.get("markdown"):
        (out_dir / "markdown.md").write_text(result["markdown"], encoding="utf-8")

    md_dir = out_dir / "markdown_out"
    md_dir.mkdir(parents=True, exist_ok=True)
    (md_dir / "input.md").write_text(result["markdown"], encoding="utf-8")
